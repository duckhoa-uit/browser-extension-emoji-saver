...
export function isBackgroundPage(): boolean {
  const isGeneratedBgcPage = location.href.startsWith(
    getURL("_generated_background_page.html")
  );
  if (isGeneratedBgcPage) return true;

  const serviceWorkerFile = getManifest().background.service_worker; // manifest@v3
  const bgcScriptFileV2 = (
    getManifest().background as chrome.runtime.ManifestV2["background"]
  )?.scripts?.[0]; // manifest@v2
...
export async function sendMessage<Request, Response = any, Error = any>({
  tabId,
  ...message
}: Message<Request> & { tabId?: number }): Promise<Response> {
  try {
    ...
    console.log("🚀 ~ file: runtime.ts:67 ~ error:", error);
  }
}
...
export function onMessage<Request, Response = unknown>(
  type: MessageType,
  getResult: OnMessageCallback<Request, Response>
) {
  ...
  chrome.runtime.onMessage.addListener(function listener(
    ...
    if (message.type === type) {
      try {
        ...
        console.log("🚀 ~ file: runtime.ts:89 ~ error:", error);
        sendResponse({ error });
      }
    }
